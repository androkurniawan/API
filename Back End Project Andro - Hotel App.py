...
# def searching_hotel():
#     data = request.get_json()
#     k = db.engine.execute(f'''select hotel.name,hotel.city,hotel.address,hotel.phone,deluxe.facility,deluxe.stock,deluxe.price,standard.facility,standard.stock,standard.price,superior.facility,superior.stock,superior.price,booking.checkout from hotel inner join deluxe on deluxe.hotel_id = hotel.id inner join standard on standard.hotel_id = hotel.id inner join superior on superior.hotel_id = hotel.id inner join booking on booking.hotel_id = hotel.id where hotel.city ilike '{data['kota_asal']}%%' and booking.checkout::text ilike '{data['tanggal_pencarian']}%%' ''')
#     x = []
#     for i in k:
#         x.append({
#             'nama_hotel':i[0],
...
#             'alamat_hotel':i[2],
#             'nomor_hotel':i[3],
#             'deluxe_fasilitas':i[4],
#             'deluxe_stock':i[5],
#             'deluxe_harga':i[6],
#             'standar_fasilitas':i[7],
#             'standar_stock':i[8],
#             'standar_harga':i[9],
#             'superior_fasilitas':i[10],
#             'superior_stock':i[11],
#             'superior_harga':i[12],
#             'tanggal_pencarian':i[13]
#         })
#     return jsonify(x)
...
# Search Avail All Room Type #
@app.route('/searchRoom', methods=['GET'])
def searchRoom():
    data = request.get_json()
    k = db.engine.execute(f'''
        select COALESCE((su.stock - sum(xx.superior)),0) as avail_superior, COALESCE((de.stock - sum(xx.deluxe)),0) as avail_deluxe, 
        COALESCE((st.stock - sum(xx.standard)),0) as avail_standard
        from booking as xx
        join superior as su on su.hotel_id = xx.hotel_id
        join deluxe as de on de.hotel_id = xx.hotel_id
        join standard as st on st.hotel_id = xx.hotel_id
        where (xx.checkin, xx.checkout) overlaps ('{data['checkin']}'::date, '{data['checkout']}'::date) and xx.hotel_id = {data['hotel_id']}
        group by su.stock, de.stock, st.stock
    ''')
...
def searchAllRoom(data):
    k = db.engine.execute(f'''
        select xx.stock as sup, yy.stock as del, zz.stock as sta
        from superior xx
        inner join deluxe yy on xx.hotel_id = yy.hotel_id
        inner join standard zz on xx.hotel_id = zz.hotel_id
        where xx.hotel_id = {data['hotel_id']}
    ''')
